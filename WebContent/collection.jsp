...
<%
	...
	ArrayList<ItemBean> list_collectionitem;
	String head="";
	if(request.getParameter("subcategory_id")!=null)
	{
		subcategory_id=request.getParameter("subcategory_id");
		SubCategoryBean subCategoryBean = subCategoryDAO.getSubCategoryDetail(subcategory_id);
		head=subCategoryBean.getName();
		list_collectionitem = (ArrayList<ItemBean>) itemDAO.showSubCollectionItem(category_id,subcategory_id);
	}	
	else
	{
		CategoryBean categoryBean = categoryDAO.getCategoryDetail(category_id);
		head=categoryBean.getCategoryName();
		list_collectionitem = (ArrayList<ItemBean>) itemDAO.showCollectionItem(category_id);
	}
	ArrayList<ItemBean> list_collection = (ArrayList<ItemBean>) itemDAO.showCollectionItem(category_id);
...
<header class="section-header section-header--large">
  <h1 class="section-header__title section-header__title--left"><%=head %></h1>
  ...
  <nav class="tags">
    <ul class="inline-list small--text-center">
      <li  class="tag--active">
        ...
        <a href="/collections/laptops?sort_by=title-ascending">All</a>
      </li>
      
      <%
      		for(int i=0;i<list_collection.size();i++){
				ItemBean itemBean = list_collection.get(i);
      %>
        
          <li>
            <a title="Show products matching tag <%=itemBean.getName() %>&quot;" href="collection.jsp?category_id=<%=itemBean.getCategory_id()%>&&subcategory_id=<%=itemBean.getSubcategory_id()%>"><%=itemBean.getName() %></a>
          </li>
      <%
      		}
      %>
...
<%
	...
	for(int i=0;i<list_collectionitem.size();i++){
		ItemBean itemBean = (ItemBean) list_collectionitem.get(i);
