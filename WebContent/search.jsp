  ...
  <nav class="sidebar-module">
    ...
    <div>
      <p><a href="#"><img src="index_files/ad.png"></a></p>
		...
		<%
			String query = "";
			String queryHead="Search for Products on Our Site";
			ArrayList<ItemBean> list_search = (ArrayList<ItemBean>) request.getAttribute("search_list");
			if(request.getParameter("query")!=null){
				query=request.getParameter("query");
				if(list_search.size()>0)
					queryHead="Your search for '"+query+"' revealed the following:";
				else
					queryHead="Your search for '"+query+"' did not yield any results.";
			}
		%>
		
        <div class="section-header section-header--large">
          <h1 class="section-header__title h2"><%=queryHead %></h1>
        </div>
        

<form action="SearchServlet" method="get" class="input-group search-bar" role="search">
  
  <input name="query" value="<%=query %>" placeholder="Search our store" class="input-group-field" aria-label="Search our store" type="search">
  <span class="input-group-btn">
	...
	<%
		...
		for(int i=0;i<list_search.size();i++){
			ItemBean itemBean = (ItemBean) list_search.get(i);
    ...
    <div class="grid__item one-fifth">
      <a href="productDetail.jsp?item_id=<%=itemBean.getItem_id() %>" title="<%=itemBean.getName()%>">
        <img src="<%=imgURL %>" alt="<%=itemBean.getName()%>">
      </a>
    </div>
  
  	<div class="grid__item four-fifths ">
    <h3><a href="productDetail.jsp?item_id=<%=itemBean.getItem_id()%>" title=""><%=itemBean.getName()%></a></h3>
